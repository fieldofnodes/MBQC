...
int main() {
  ...
  // Playing with rate of occurrences
  // Is there a Bayesian approach here, using this as
  // as the likelihood data?
  std::vector<std::vector<int>> resultsVector;
    ...
    for (int i = 0; i < 1000;i++){
      measuredQubitsOutcomes = measureGraph(
            ...
            qubitAngles,
            numRows);
      resultsVector.push_back(measuredQubitsOutcomes);
  }    



  std::vector<double> resultsRatio;
  resultsRatio = calculateColumnRatios(resultsVector);
  for (const auto& ratio : resultsRatio) 
  {
                  std::cout << ratio << ' ';
  }
